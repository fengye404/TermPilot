...
- 一套本地优先的会话模型，会话标题、状态和输出保留在 agent 所在电脑
- 一套设备级配对与授权模型，用于浏览器与 agent 之间的加密通信
...
- 访问模型: 设备级配对后，浏览器与 agent 之间的会话消息加密传输
- relay 持久化: 默认只保留配对、grant 与审计元数据，不保存会话标题、cwd 或终端输出
- 存储: 默认内存存储，可通过 `DATABASE_URL` 切换到 PostgreSQL，用于 relay 元数据

当前这套实现是完整的一条主路径：relay、配对、受管理会话、移动端查看和轻控制已经连成一个可以直接使用的产品形态。

## 安全模型

- 会话数据本地优先：会话标题、cwd、shell、状态细节和终端输出保留在 agent 所在电脑。
- 已配对客户端会在配对阶段与 agent 交换公钥，并获得单设备范围的 access token。
- 配对时 agent 会打印设备指纹；浏览器完成绑定后会展示同一指纹用于核对。
