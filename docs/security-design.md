...
这份文档描述 **当前已实现** 的安全模型，以及当前部署形态下的设计边界。

它的目标是把当前代码里的安全设计讲清楚，让部署方式、数据流和密钥模型保持一致表述。
...
- 会话内容留在 agent 所在电脑
- 浏览器与 agent 之间的业务消息以加密信封经过 relay
...
- relay 负责交付 Web UI，并承担配对与授权的中心路由
- 浏览器通过 relay 获取配对材料，并与 agent 完成设备级绑定
- 已配对浏览器与 agent 的业务消息使用当前长期密钥关系进行加密传输

在这个模型下，relay 的职责被压缩到交付、路由与最小元数据管理，而会话内容、输出和状态详情保留在端侧。
