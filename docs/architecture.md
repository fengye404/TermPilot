...
这份文档描述的是 **当前已经实现** 的架构，不是长期目标架构。

如果你想看本地优先、零知识 relay 和端到端加密的未来方向，请读 [产品演进路线图](./roadmap.md)。
